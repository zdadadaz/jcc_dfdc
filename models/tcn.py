...
# -*- coding: utf-8 -*-
"""
Created on Sun Feb  2 11:15:54 2020
...
from keras.layers import Multiply, Concatenate, concatenate
...
def ED_TCN(n_nodes, conv_len, n_classes, n_feat, max_len, 
            loss='binary_crossentropy', causal=False, 
    ...
    # Output FC layer
    model = TimeDistributed(Dense(n_classes, activation="sigmoid" ))(model)
...
def Test_time_series(n_nodes, conv_len, n_classes, n_feat, max_len, 
            loss='binary_crossentropy', causal=False, 
            optimizer="rmsprop", activation='norm_relu',
            return_param_str=False):
    n_layers = len(n_nodes)
...
def BidirLSTM(n_nodes, n_classes, n_feat, max_len=None, 
                causal=True, loss='binary_crossentropy', optimizer="adam",
                return_param_str=False):
